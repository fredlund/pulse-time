...
%%% Created :  3 Aug 1999 by Hakan Huss <hakan@erlang.ericsson.se>
%%%----------------------------------------------------------------------
...
uninitialized({reset, State, Floor}, {ENo, unknown_floor}) ->
    lift_scheduler:set_controller(ENo, self()),
...
open(close, {ENo, Floor}) ->
    sys_event:close(ENo),
    lift_scheduler:closed(ENo, Floor),
...
moving({approaching, NewFloor}, {ENo, Floor}) ->
    ...
    case lift_scheduler:approaching(ENo, NewFloor) of
	{ok, stop} ->
