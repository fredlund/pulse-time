%%%----------------------------------------------------------------------
%%% File    : lift_scheduler.erl
%%% Author  : Hakan Huss <hakan@erlang.ericsson.se>
%%% Purpose : Elevator lift_scheduler.
%%% Created :  4 Aug 1999 by Hakan Huss <hakan@erlang.ericsson.se>
%%%----------------------------------------------------------------------

-module(lift_scheduler).
...
%%----------------------------------------------------------------------
%% start_link()
%%  Starts the lift_scheduler.
%%----------------------------------------------------------------------
start_link() ->
    gen_server:start_link({local, lift_scheduler}, lift_scheduler, [], []).

%%----------------------------------------------------------------------
%% set_controller(ENo, EPid)
%%  Informs the lift_scheduler that the control porcess EPid has been 
%%  started for elevator ENo.
%%----------------------------------------------------------------------
set_controller(ENo, EPid) ->
    gen_server:cast(lift_scheduler, {set_controller, ENo, EPid}).

%%----------------------------------------------------------------------
%% f_button_pressed(Floor)
%%  Informs the lift_scheduler that the call button on the floor Floor
%%  has been pressed.
%%----------------------------------------------------------------------
f_button_pressed(Floor) ->
    sys_event:f_button_pressed(Floor),
    gen_server:cast(lift_scheduler, {f_button, Floor}).

%%----------------------------------------------------------------------
%% e_button_pressed(ENo, Floor)
%%  Informs the lift_scheduler that the elevator button for the floor Floor
%%  has been pressed in elevator ENo.
%%----------------------------------------------------------------------
e_button_pressed(ENo, Floor) ->
    sys_event:e_button_pressed(ENo, Floor),
    gen_server:cast(lift_scheduler, {e_button, ENo, Floor}).

%%----------------------------------------------------------------------
%% passing(Floor)
%%  Informs the lift_scheduler that elevator ENo is passing the floor Floor.
%%----------------------------------------------------------------------
passing(ENo, Floor) ->
    gen_server:cast(lift_scheduler, {passing, ENo, Floor}).

%%----------------------------------------------------------------------
%% passing(Floor)
%%  Informs the lift_scheduler that elevator ENo has stopped at the floor Floor.
%%----------------------------------------------------------------------
open(ENo, Floor) ->
    gen_server:cast(lift_scheduler, {open, ENo, Floor}).

%%----------------------------------------------------------------------
%% closed(ENo, Floor)
%%  Informs the lift_scheduler that elevator ENo has closed the doors on floor
%%  Floor.
%%----------------------------------------------------------------------
closed(ENo, Floor) ->
    gen_server:cast(lift_scheduler, {closed, ENo, Floor}).

%%----------------------------------------------------------------------
%% approaching(ENo, Floor)
%%  Informs the lift_scheduler that elevator ENo is approaching the floor Floor,
%%  expects a reply indicating whether or not to stop.
%%  Uses catch to handle the possibility of a timeout.
%%----------------------------------------------------------------------
approaching(ENo, Floor) ->
    catch gen_server:call(lift_scheduler, {approaching, ENo, Floor}, 200).
...
%%----------------------------------------------------------------------
%% init([])
%%  Initializes the lift_scheduler by retrieving the elevator states for all
...
init(_) ->
	...
	case whereis(elev_sup) of
		...
		[];
	    _Pid ->
	    Children = supervisor:which_children(elev_sup),
	    io:format("supervisor ~p has children~n~p~n",[self(),Children]),
